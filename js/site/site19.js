...
function sell_product(data){
  ...
  //  console.log('data', data);
    ...
    $.ajax({
        ...
        success: function (json) {
            {
               console.log('product selled Start drowSellCount v last v23');
               close_sell_product();
            }
        }
    });
...
function drowTableReport(data) {
    ...
    console.log('products connect obj. -', products);
    var tmpReport  = '';
...
function haveConnect() {
    console.log('haveConnect v 1.5');
     var element =  $('<div style="position: fixed; font-size: 22px; color: red;" class="pull-left"><span class="glyphicon glyphicon-record"></span></div>');
                    $('#report_place').append(element);
...
function isConnect () {
    ...
    $.ajax({
        ...
        success: function (json) {
            {
                console.log('json-. connect', json)
               if (json) {
                   var element =  $('<div id="have_connect_indicator'+json+'" style="position: fixed; font-size: 22px; color: #57b257;" class="pull-left"><span class="glyphicon glyphicon-record"></span></div>');
                    $('#report_place').append(element);
                   setTimeout('removeConnectIndicator('+json+');', 4000)
               }
            }
        }
    });
...
function removeConnectIndicator(id) {
      $("#have_connect_indicator"+id).remove();
}
//haveConnect();